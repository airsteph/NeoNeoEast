# NeoNeoEast · Foreign Vocabulary Assistant for Chinese Reading

![NeoNeoEast Demo Screenshot](demo-screenshot.png)

A zero-config Chrome extension
...
> Install and go. No account, no network translation, no data upload — all learning records stay in your own browser.
...
- **Hover Cards**: Definitions, phonetics (English) / part-of-speech (Japanese), example sentences (native + Chinese translation), and quick "Got it / Not yet" buttons.
- **Spaced Repetition**: Words marked "Not yet" enter a review queue. Daily review with caps and rollover — no penalties, no forgetting curves.
...
4. Click **Load unpacked**, then select the unzipped `NeoNeoEast-v2` folder.
...
└── icons/                   # Pixel globe icons (16/32/48/128)
```

## 🔒 Privacy

This extension makes **no network requests and sends no data anywhere**. Dictionaries are bundled locally with the extension. All learning records are stored in the browser's `chrome.storage.local`. Uninstalling removes everything.

## 🛠️ Tech Stack

Vanilla JavaScript / HTML / CSS — no build step, no third-party runtime dependencies. Fonts (Space Mono / Space Grotesk / Doto) are loaded via Google Fonts.
